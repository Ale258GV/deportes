...
def change_password_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Listo, Actualizada la contraseña')
        else:
            messages.error(request, 'Datos no coincidentes')
    else:
...
def profile_view(request, teacher_id):
    profile, created = Profile.objects.get_or_create(
        user=request.user,
        defaults={
            'description':'Aún no hay una descripción disponible',
        }
    )
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.has_changed():
                ...
                messages.info(request, 'No hay cambios')
        else:
